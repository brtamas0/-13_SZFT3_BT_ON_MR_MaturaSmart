...
const handleResetPassword = async () => {
  ...
  try {
    const response = await fetch('http://backend.maturasmart.hu/api/reset-password', {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      throw new Error(data.message || 'Hiba a jelszóvisszaállításkor.')
    }

    successMessage.value = 'Sikeres jelszócsere! Máris átirányítunk a bejelentkezéshez...'
      ...
      <div class="grid md:grid-cols-2 rounded-3xl overflow-hidden border border-white/10 shadow-2xl bg-[#020617]">
        ...
        <div class="bg-[#0b1121] p-8 md:p-12 flex flex-col justify-center w-full">
            ...
            <div class="text-center mb-8">
                ...
                <h2 class="text-3xl font-bold text-white">Új jelszó megadása</h2>
                <p class="text-slate-400 text-sm mt-2">Kérlek add meg az új jelszavadat.</p>
            </div>
            ...
            <form v-if="!successMessage" @submit.prevent="handleResetPassword" class="space-y-4 w-full max-w-sm mx-auto">
                
                <div class="space-y-1">
                    <label class="text-xs font-bold uppercase text-slate-500 ml-1">Új Jelszó</label>
                    <input 
                        v-model="password"
                        type="password" 
                        placeholder="••••••••"
                ...
                <div class="space-y-1">
                    <label class="text-xs font-bold uppercase text-slate-500 ml-1">Jelszó újra</label>
                    <input 
